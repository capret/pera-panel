...
**World settings → Player permissions** shows known IDs from raw player save paths, cached-owner records, authenticated lines in an uploaded shard `server_log.txt` (up to 2 MiB), and players seen by the running server. New authenticated joins are persisted even when the dashboard is closed. With the dashboard open, bounded console queries also discover connected accounts. Character recovery works from existing save folders and does not depend on a live console query or the path-encoding API. The list refreshes without overwriting unsaved settings. Candidate buttons only add IDs to the form; stop the world and save settings to apply them. Existing permission lists are never replaced by imported candidates.

Cached-owner entries are labelled as unverified. A raw save folder ending in `_` is matched against known account IDs both with and without that one suffix. Only an unambiguous match supplies the account label; actual IDs from authentication or permission lists are never trimmed. Refreshing characters removes stale save-only candidates created by the old literal-folder parser. Encoded and offline character directories appear separately without inventing Klei IDs. Optional plain-text `.meta` files, including DST's NUL-terminated files, supply character names; unknown/compressed metadata is left untouched. Discovery is limited to 1,000 records and a bounded directory scan, and does not execute uploaded Lua.

If joining still creates a new character, use **Backups & rollback → Recover an imported character**:

1. If you have already joined online and created a character, stop the world. Otherwise join online in the same shard and create a temporary character first, then stop it.
2. Click **Refresh saved characters**, choose the original character, and select **Destination character save**. Destination choices are other existing character folders in the same shard and world session; the live console does not need to be available.
3. Check the exact source and destination paths in the confirmation dialog, then confirm the world name. For a raw `KU_example_` folder the account label may be `KU_example`; the actual directory suffix is preserved.
...
The panel sends `c_shutdown(true)` to each shard, flushes the command and closes that shard's console input pipe **before** waiting for process exit. Leaving stdin open until after `wait()` can deadlock a child that has logged `Shutting down` but is waiting for its console reader to receive EOF. Both shards receive the shutdown request and EOF before either is awaited. The log readers continue draining stdout during shutdown.
...
To keep the current save on your computer, stop the world, open **Backups & rollback → Download saved game**, and click **Download save ZIP**. The ZIP contains a single `Cluster_<world-id>` folder with surface/cave save data (including player saves), mod options, world-generation settings, and portable INI configuration. It excludes server tokens, passwords, panel metadata, logs, and downloaded mod binaries. Upload it again through **Import local save** to restore its progress and mod settings; the destination world's panel settings and credentials are kept. Downloads use the same limits as imports (256 MiB ZIP, 2 GiB expanded, 512 MiB per file) and reject incomplete or linked saves. The world remains stopped, and the temporary export is removed when the response closes.
